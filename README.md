# 🌐 Priyanshu Maurya – Developer Portfolio

Hi 👋 I’m **Priyanshu Maurya**, a Software Developer passionate about building modern, responsive web applications using React, JavaScript, and Tailwind CSS.

This is my personal portfolio website showcasing my projects, skills, and contact information.

---

## 🚀 Live Demo

🔗 **Website:**  
   https://priyans-portfolio.netlify.app/  

---

## 🛠 Tech Stack

- React.js      
- EmailJS (Contact Form Integration)  
- HTML5 & CSS3  

---

## 📂 Featured Projects

### 💳 ApnaPaytm  
A React-based Paytm UI clone built using Tailwind CSS.  
🔗 Live: https://priyanshu-digipay.netlify.app/  
🔗 GitHub: https://github.com/Priyanshu-Mm/ApnaPaytm  

---

### 💼 Job Portal (MERN Stack)  
A full-stack job portal application built with MongoDB, Express, React, and Node.js.  
🔗 GitHub: https://github.com/Priyanshu-Mm/job-portal-mern  

---

### 🧠 Grow My Therapy Clone  
Built as a technical assessment using Next.js and Tailwind CSS with responsive UI design.  
🔗 Live: https://grow-my-therapy-clone.netlify.app/  
🔗 GitHub: https://github.com/Priyanshu-Mm/grow-my-theropy-clone  

---

## ✨ Features

- Responsive Design
- Smooth UI/UX
- Working Contact Form (EmailJS Integration)
- Social Media & Coding Profile Links
- Clean Component-Based Architecture

---

## 📬 Contact Me

- GitHub: https://github.com/Priyanshu-Mm  
- LinkedIn: https://www.linkedin.com/in/priyanshu-maurya-a87b88302  
- LeetCode: https://leetcode.com/u/Priyanshu_001/  
- GeeksforGeeks: https://www.geeksforgeeks.org/profile/maupriyavb3b  

---

## ⚙️ Installation & Setup

```bash
git clone https://github.com/Priyanshu-Mm/Portfolio.git
cd Portfolio
npm install
npm run dev
